...
def background_thread():
	...
	while True:
		...
		#functionality 2: continuously stream data of the past 4 hours? = 4 x 60 = 240 data points 
...
def getDates(mydates):
    ...
    if startDate == "" or endDate == "":
    	emit('lineChart', getLogCSV.getCount_linechart(databaseFilepath, startTime, endTime))
    	emit('pieChart', getLogCSV.getCount_piechart(databaseFilepath, startTime, endTime))
    	#emit('historicalTable', )
	
    elif startDate != "" and endDate != "":
    	startDate = datetime.strptime(str(startDate), '%Y-%m-%d')
    	endDate = datetime.strptime(str(endDate), '%Y-%m-%d')
    	print('emitted')
    	emit('lineChart', getLogCSV.getCount_linechart(databaseFilepath,startTime, endTime, startDate, endDate))
    	emit('pieChart', getLogCSV.getCount_piechart(databaseFilepath, startTime, endTime, startDate, endDate))
    	emit('historicalTable', getLogCSV.getCount_table(databaseFilepath, startTime, endTime, startDate, endDate))
...
@socketio.on('disconnect', namespace = '/pilab')
def pilab_disconnect():
	pass
